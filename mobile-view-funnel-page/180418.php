...
<html lang="en"><head>
	...
	<style>
		html,body {margin: 0; padding: 0; color: #333; font-family: "HelveticaNeue-Light", "Helvetica Neue Light", "Helvetica Neue", Helvetica, Arial, "Lucida Grande", sans-serif; font-weight: 300; font-size:16px;}
    ...
    .shield li{background: url("images/favicon.png") no-repeat 9px 7px; width: 270px; list-style: none;
               padding-left: 58px; font-size: 15px; margin-bottom: 30px; background-size:40px;}
    .points-container{width: 670px; margin: auto; padding: 10px 20px 1px 20px; margin-bottom: 30px;-webkit-box-shadow: 0px 0px 17px 2px rgba(0,0,0,0.75);-moz-box-shadow: 0px 0px 17px 2px rgba(0,0,0,0.75);box-shadow: 0px 0px 17px 2px rgba(0,0,0,0.75);}
    .shield{margin-bottom: 0; padding-left: 0;}
    .font18{font-size: 18px; margin-top: 0;}
    .floatLeft{float: left;}
    .floatRight{float: right;}
    .center {text-align: center;}
    .inThis{margin-top: 40px;}

    /* semi full screen */
    .full_screen {position: absolute; left: 50%; top: 0; transform: translate(-50%); width: 70vw !important;}
    .visible {visibility: visible !important; z-index: 100000;}
    .bg {visibility: hidden; z-index: 99999; position: absolute; left: 0; top:0; width: 100%; height: 100%; background-color: black;}
...
<body>
<img id="close-window-btn" src="images/close-window.png" style="visibility: hidden; position: absolute; top: 0; right: 0; height: 50px; cursor: pointer;">
<div id="ad-div">
...
<?php if($n=='ob' || $n=='obm'){ ?><p>Advertorial</p><?php } ?>
...
<!-- Start OOYALA 
Gundry_Total_Restore_VSL_180302A-->
          <style type="text/css">
            .oo-control-bar{
             visibility: hidden;
             pointer-events: none;
      ...
      OO.ready( function() {
      	...
      	// Create Player
      	window.pp = OO.Player.create( 'LimelightEmbeddedPlayerFlash', 'w5dmdtZTE6VgBaJafGRexYnpr5wJdCj9', {
...
<!-- End OOYALA -->
            ...
            <div id="CTALink" align="center">
          	  <a href="assessment180418.php" id="button" class="orderpage_btnB" target="_blank">NEXT STEP</a>
            </div>
          <img id="full_screen_btn" src="images/full_screen.png" style="float: right; height: 22px; margin-top: -19px; cursor: pointer;">
          <img src="images/clickHere_graphic.png" style="float: right; height: 53px; margin-top: -42px;">
...
<!-- main content -->
   ...
   <?php if($n=='ob' || $n=='obm'){ ?>
   <div class="inThis">
    <p class="center font18">In this video, you will learn about...</p>
      <div class="points-container">
        <ul class="shield floatRight shield-top">
...
<script>
  if(!$.cookie('returning180308A')){
    ...
    // set following cookie to expire same as Button timer but in DAYS (seconds/86400)
    $.cookie("fullscreen_cookie", 1, { expires : 0.026261574074074 });
  } 
  else {
    // when testing below cookie on local, path is '/TheNewGutFix'
    $.removeCookie("fullscreen_cookie", { path: '/' });
    $("#CTALink a").show();
  }
</script>
<script type="text/javascript">
  var fullscreenTimeout
  function insertOrderBtn() {
    $('#fullscreen-BG').append('<div id="fullscreen-cta-btn" style="z-index: 100000; position: absolute; bottom: 15vh; left: 50%; transform: translate(-50%)"><a href="assessment180418.php" align="center" style="height: 25px; padding-top:10px; color: white; position: absolute; left: 50%; transform: translate(-50%);" class="orderpage_btnB" target="_blank">NEXT STEP</a></div>')
  }

  $('#full_screen_btn').click(function(){ 
    $('#LimelightEmbeddedPlayerFlash').addClass('full_screen')
    $('#close-window-btn').addClass('visible')
    $('#fullscreen-BG').addClass('bg visible')
    var runningVideoTime = $('[data-reactid=".0.0.3.3.1.$timeDuration.0"]').html()
    var pausedVideoTime = $('[data-reactid=".0.0.5.3.1.$timeDuration.0"]').html()
    var videoTime = Object.is(runningVideoTime, undefined) ? pausedVideoTime : runningVideoTime;
    var array = videoTime.split(":")
    var totalTime = parseInt(array[0]) + parseInt(array[1])/60

    if (totalTime < 37.816 && $.cookie('fullscreen_cookie')) {
        // Button PRICE: 37:49
        var delay = <?php if(isset($_GET['del'])) echo $_GET['del']; else echo '37.816'; ?> * 60000;
        if ($('#LimelightEmbeddedPlayerFlash').hasClass('full_screen')) {
  ...
  $('#close-window-btn').click(function() {
    ...
    $('#LimelightEmbeddedPlayerFlash').removeClass('full_screen')
    $('#fullscreen-BG').removeClass('bg')
    $('#fullscreen-cta-btn').remove()
